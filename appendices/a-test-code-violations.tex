...
\label{appendix:class-violations}
...
\label{appendix:method-violations}

As violações desta seção aplicam-se a um método de teste. O analisador exclui apenas o método afetado, de modo que os demais testes da classe possam seguir no pipeline. Quando a mesma construção ocorre no método de configuração implícita, o analisador registra uma violação de classe, pois a extração da configuração compartilhada ficaria comprometida para todos os testes.
...
\subsection{Criação de Thread, Socket ou URL}

Ocorre na instanciação de \texttt{Thread}, \texttt{Socket} ou \texttt{URL}. O uso direto de concorrência ou rede na configuração constitui uma má prática para testes unitários quando introduz dependência externa, disputa ou espera não controlada. No trecho, \texttt{start} inicia uma execução paralela cuja conclusão não é determinada pela posição textual dos comandos. Redistribuir o teste ou extrair essa criação para um método compartilhado pode mudar a quantidade de instâncias, o momento da conexão ou a relação entre as threads. A violação evita que a refatoração amplifique esses efeitos e produza resultados não determinísticos.

\begin{violationexample}
new Thread().start();
\end{violationexample}

\subsection{Chamada de reflexão}

Ocorre na presença de \texttt{Class.forName}, \texttt{getDeclaredMethod}, \texttt{getDeclaredField}, \texttt{getMethod} ou \texttt{getField}. Essa violação evita que a refatoração invalide nomes usados de forma reflexiva. No trecho, \texttt{"Banco"} é uma cadeia de caracteres, e não uma referência de tipo que possa ser atualizada pelo analisador. O mesmo ocorre com nomes de métodos e atributos fornecidos como texto. Se a geração mudar pacotes, classes ou membros, a chamada pode deixar de localizar o alvo sem produzir erro de compilação. Como o Róża não resolve nem reescreve dependências reflexivas, recusa o método para preservar seu comportamento.

\begin{violationexample}
Class.forName("Banco");
\end{violationexample}

\subsection{Chamada com efeito colateral de arquivo, rede ou tempo}

Ocorre no uso de \texttt{Files}, \texttt{Paths}, \texttt{DriverManager}, \texttt{URL}, \texttt{Socket}, \texttt{Clock}, \texttt{Instant}, \texttt{LocalDate} ou \texttt{LocalDateTime}. Dependências diretas de arquivo, banco de dados, rede ou relógio constituem uma má prática em testes unitários quando tornam o resultado dependente do ambiente ou do instante da execução. No trecho, \texttt{readAllLines} depende da existência e do conteúdo de um arquivo externo. Extrair ou compartilhar essa chamada pode mudar o momento e a quantidade de acessos, além de fazer testes antes isolados dividirem o mesmo recurso. A violação evita que a refatoração altere esses efeitos e aumente o caráter não determinístico dos testes.

\begin{violationexample}
Files.readAllLines(path);
\end{violationexample}

\subsection{Chamada a System}

Ocorre no uso de \texttt{System.currentTimeMillis}, \texttt{System.nanoTime}, \texttt{System.exit} ou \texttt{System.getProperty}. Essas chamadas acessam tempo, propriedades ou controle global da máquina virtual, o que constitui uma má prática quando torna o teste dependente do ambiente ou capaz de encerrar o processo. No trecho, duas execuções de \texttt{currentTimeMillis} podem produzir valores distintos mesmo em testes com código igual. Mover ou compartilhar a chamada altera o instante em que o valor é obtido e pode mudar o resultado observado. Por isso, o Róża recusa o método em vez de refatorar uma dependência global cujos efeitos não consegue preservar.

\begin{violationexample}
System.currentTimeMillis();
\end{violationexample}

\subsection{Chamada assíncrona}

Ocorre no uso de \texttt{CompletableFuture} ou \texttt{Executors}. A execução assíncrona constitui uma fonte de não determinismo quando o teste não controla explicitamente a conclusão e os recursos utilizados. No trecho, a tarefa submetida pode continuar enquanto os comandos seguintes já estão em execução. Extrair a criação do executor para a configuração compartilhada pode ainda fazer testes distintos usarem o mesmo pool e interferirem entre si. Como a ordem textual deixa de determinar a ordem dos efeitos, a violação evita que a refatoração altere a concorrência e produza falhas intermitentes.

\begin{violationexample}
Executors.newSingleThreadExecutor().submit(tarefa);
\end{violationexample}
