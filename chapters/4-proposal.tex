...
Na Equação~\ref{equation:lcs-similarity}, $|\alpha|$ e $|\beta|$ correspondem ao número de comandos da configuração de acessórios de cada teste, e $|LCS(\alpha, \beta)|$ corresponde ao número de comandos da subsequência comum mais longa.
...
Na Equação~\ref{equation:lccss-similarity}, $|\alpha|$ e $|\beta|$ também correspondem ao número de comandos da configuração de acessórios, e $|LCCSS(\alpha, \beta)|$ corresponde ao comprimento do prefixo comum contíguo.
...
\begin{table}[htb]
	...
	\begin{tabular}{lccc>{\raggedright\arraybackslash}p{3.4cm}c}
		...
		\hline
		JPlag & tokens & assimétrica & arquivos & sensibilidade (tokens) & Não \\
		Simian & texto & assimétrica & arquivos & limiar de linhas ($\geq 2$) & Não \\
